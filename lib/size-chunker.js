(function () {
   ...
   SizeChunker.prototype._startIfNeededAndPushData = function(buf, done) {
   ...
   SizeChunker.prototype._transform = function (chunk, encoding, done) {
      ...
      var _do_transform = function () {
         ...
         if (this._bytesPassed + chunk.length < this._chunkSize) {
            ...
            this._startIfNeededAndPushData(chunk, done);
         } else {

            remainder = bytesLeave - chunk.length;

            if (remainder === 0) {
               this._startIfNeededAndPushData(chunk, function() {
                  this._finishChunk(done);
               }.bind(this));
            } else {
               this._startIfNeededAndPushData(chunk.slice(0, bytesLeave), function() {
